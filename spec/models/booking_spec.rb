...
RSpec.describe Booking, type: :model do
  ...
  context 'bookings cannot overlap' do
    it 'overlaps' do
      listing.bookings.create!(attributes_for(:booking, guest_id: user.id, guests: 1,
                                                        start_date: '01/01/2022'.to_date,
                                                        end_date: '07/01/2022'.to_date))

      booking_week2 = listing.bookings.build(attributes_for(:booking, guest_id: user.id, guests: 1,
                                                                      start_date: '01/01/2022'.to_date,
                                                                      end_date: '07/01/2022'.to_date))

      expect(booking_week2.no_booking_overlap).to be_truthy
    end
    it 'does not overlap' do
      ...
      booking_week2 = listing.bookings.build(attributes_for(:booking, guest_id: user.id, guests: 1,
                                                                      start_date: '08/01/2022'.to_date,
                                                                      end_date: '14/01/2022'.to_date))

      expect(booking_week2.no_booking_overlap).to be_nil
    end
  end
